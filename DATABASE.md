...
### Säuglings-Anamnese (A16, seit 19.07.2026) — VORLÄUFIG
Eltern-Fremdanamnese für 0–24 Monate. Weiche: Schritt `patient-typ` (PT-001) direkt nach der Tiefe-Wahl; bei `saeugling` blenden sich alle 12 Erwachsenen-Module aus (`isVisible` je Schritt), sichtbar bleiben: Willkommen → Persönliches (= Daten des KINDES, Hinweis PT-003) → Tiefe → Für wen? → **Ihr Kind** (`step-saeugling`) → Befunde → Zusammenfassung → Konto. Datenmodul `data/A16_saeugling_eltern.js`: 9 Abschnitte (Anliegen nach Lindebergs-Kinderosteopathie-Angebot OST-PÄD OAM / Schwangerschaft / Geburt / erste Wochen / Stillen-Verdauung / Schreien-Schlaf-Regulation (Dreier-Regel, Wessel 1954) / Haltung-Entwicklung / Vorsorge / Sicherheit), 51 Fragen, 6 Säuglings-Red-Flags (jede „ja" → Stop-Overlay via registerRedFlagSource, z.B. Fieber <3 Monate, Fontanelle, Atemaussetzer→112). Regelwerk **v2026.07.19-2** (SHA-256 78183e54…, 645 Fragen) in Supabase publiziert. Status: vorläufig — fachlicher Feinschliff durch das Kinderosteopathie-Team (Advisory Board); Schwangeren-Anamnese (eigener Fragebogen) ist der logische nächste Ausbau.
